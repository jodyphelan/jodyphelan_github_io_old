...
while(<F>){
	my ($sample,$hospital,$div,$date) = split /\s+/,$_;
  $date =~ tr/\//-/;
  $meta{$sample}{'hosp'} = $hospital;
	$meta{$sample}{'div'} = substr($div,1);
...
$hosp{"SLH"} = "green";
my %div;
$div{"Community"} = "circle";
$div{"Inpatient"} = "square";
$div{"Outpatient"} = "diamond";
my %date;

my %coords;
$coords{"SGT"} = "[51.503649,-0.087804]";
$coords{"SKH"} = "[51.467132,-0.090801]";
$coords{"SLH"} = "[51.457227,-0.013589]";
  ...
  if ($sample =~ /node/){
    ...
    foreach my $key (keys %newNodes){
      if ($newNodes{$key} eq $sample){
        $children .= "\t\t\t{\"id\": \"$key\",\"col\":\"$hosp{$meta{$key}{'hosp'}}\",\"shape\":\"$div{$meta{$key}{'div'}}\",\"date\":\"$meta{$key}{'date'}\",\"coords\":$coords{$meta{$key}{'hosp'}}},\n";
      }
    }
    $children = substr($children,0,length($children)-2);
    $nodeLine .= "$children\n\t\t]},\n";
  } else {
	   $nodeLine .= "\t\t{\"id\": \"$sample\",\"col\":\"$hosp{$meta{$sample}{'hosp'}}\",\"shape\":\"$div{$meta{$sample}{'div'}}\",\"date\":\"$meta{$sample}{'date'}\",\"coords\":$coords{$meta{$sample}{'hosp'}}},\n";
  }
